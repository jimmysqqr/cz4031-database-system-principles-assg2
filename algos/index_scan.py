...
def index_scan(plan, output):
    ...
    if plan["Node Type"]=="Index Scan" or plan["Node Type"] == "Index Only Scan":
        ...
        if "Index Cond" in plan:
            output_string += ", based on the following condition(s): " + plan["Index Cond"].replace('::text', '')

        # Check if there are extra filters on the output of the index scan
        if "Filter" in plan:
            output_string += ". Index scan output is further filtered based on these condition(s): " + plan["Filter"].replace('::text', '')
